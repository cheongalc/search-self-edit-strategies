...
MODEL=$REPO_ROOT/squad/models/Qwen3-8B/baseline/rewrite/iter1  # model to use for data generation. For evaluation, set to the model to be evaluated. For RL training, set to the (n-1)'th RL checkpoint.
...
DATASET_OUT="$REPO_ROOT/squad/data/Qwen3-8B/baseline/rewrite/iter2/train_thinking.json"

NUM_ARTICLES=50
START_INDEX=0
NUM_SELF_EDIT_TEMPLATES=1
NUM_COMPLETIONS_PER_TEMPLATE=15
MAX_NEW_TOKENS=12000

VLLM_LOG_PATH="$REPO_ROOT/squad/logs/Qwen3-8B/vllm_create_self_edits_baseline_rewrite_iter2_train.log"
